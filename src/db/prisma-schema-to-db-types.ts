...
const mapPrismaToTsType = (prismaType: string, prismaEnums: DMMF.DatamodelEnum[], isList: boolean): string => {
  ...
  if (prismaEnums.length > 0) {
    for (const en of prismaEnums) {
      ...
      for (const value of en.values) {
        enumContent += `  ${value.name} = '${value.dbName}',\n`;
      }
      enumContent += '}\n\n';
    }
  }
...
const createTableType = (modelName: string, modelFields: DMMF.Field[], prismaEnums: DMMF.DatamodelEnum[]): string => {
  ...
  for (const field of modelFields) {
    ...
    const tsType = mapPrismaToTsType(field.type, prismaEnums, field.isList);
